...
#define gpu or cpu use with device
...
netG.to(device) #send model to GPU
...
netD.to(device) #send model to GPU
...
if __name__ == '__main__':
  ...
  for epoch in range(25): # We iterate over 25 epochs.
      ...
      for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
          ...
          input = Variable(real).to(device)  # We wrap it in a variable and move data to GPU
          target = Variable(torch.ones(input.size()[0])).to(device) # We get the target and move data to GPU
          ...
          noise = Variable(torch.randn(input.size()[0], 100, 1, 1)).to(device) # We make a random input vector (noise) of the generator and move data to GPU
          ...
          target = Variable(torch.zeros(input.size()[0])).to(device) # We get the target and move data to GPU
