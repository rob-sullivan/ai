import logging #for printing stats
...
from keras.datasets import fashion_mnist #keras for downloading and working with F-MNIST dataset
from keras.utils import np_utils

class SimpleCNN(object):
    def __init__(self, learning_rate, num_epochs, beta, batch_size):
        self.learning_rate = learning_rate # learning rate for optimsation
        self.num_epochs = num_epochs # number of times it takes to train the network
        self.beta = beta #0 to 1 value that controls strength of L2-penalty
        self.batch_size = batch_size # number of images to train per step

        #location for parameters and stat logs
        self.save_dir = 'saves'
        self.logs_dir = 'logs'
...
#there are two methods that can be used to build a network, the build method and the fit method.
#build method
def build(self, input_tensor, num_classes):
    """
    Builds a cnn according to input shape and number of classes
    archateture is fixed.
    Args:
    ...
    """
    #tf placeholders are empty variables to be used later
    with tf.name_scope('input_placeholders'):
        self.is_training = tf.placeholder_with_default(True, shape=(), name='is_training')

    #we create 3 kinds of layers of for our cnn
    ...
    #could add batch-normalization layers to improve the stability of training

    #add the fully-connected layers that lead to the output
...
if __name__ == "__main__":
    ...
    simple_cnn.fit(X_train=X_train,
                ...
                y_valid=y_test)
# run python cnn.py
# can also run tensorboard --logdir=logs/ to see a graph of the model's accuracy and loss