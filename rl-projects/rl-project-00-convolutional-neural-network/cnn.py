...
class SimpleCNN(object):
    ...
    def build(self, input_tensor, num_classes):
        ...
        """
        with tf.compat.v1.name_scope("input_placeholders"):
            self.is_training = tf.compat.v1.placeholder_with_default(True, shape=(), name="is_training")

        with tf.compat.v1.name_scope("convolutional_layers"):
            conv_1 = tf.compat.v1.layers.conv2d(
                input_tensor,
                filters=16,
                kernel_size=(5, 5),
                strides=(1, 1),
                ...
                kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (self.beta)),
                name="conv_1")
            conv_2 = tf.compat.v1.layers.conv2d(
                conv_1,
                filters=32,
                kernel_size=(3, 3),
                strides=(1, 1),
                ...
                kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (self.beta)),
                name="conv_2")
            pool_3 = tf.compat.v1.layers.max_pooling2d(
                conv_2,
                pool_size=(2, 2),
                strides=1,
                padding="SAME",
            ...
            drop_4 = tf.compat.v1.layers.dropout(pool_3, training=self.is_training, name="drop_4")

            conv_5 = tf.compat.v1.layers.conv2d(
                drop_4,
                filters=64,
                kernel_size=(3, 3),
                strides=(1, 1),
                ...
                kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (self.beta)),
                name="conv_5")
            conv_6 = tf.compat.v1.layers.conv2d(
                conv_5,
                filters=128,
                kernel_size=(3, 3),
                strides=(1, 1),
                ...
                kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (self.beta)),
                name="conv_6")
            pool_7 = tf.compat.v1.layers.max_pooling2d(
                conv_6,
                pool_size=(2, 2),
                strides=1,
                padding="SAME",
            ...
            drop_8 = tf.compat.v1.layers.dropout(pool_7, training=self.is_training, name="drop_8")

        with tf.compat.v1.name_scope("fully_connected_layers"):
            flattened = tf.compat.v1.layers.flatten(drop_8, name="flatten")
            fc_9 = tf.compat.v1.layers.dense(
                flattened,
                units=1024,
                activation=tf.nn.relu,
                kernel_regularizer=tf.keras.regularizers.l2(l=0.5 * (self.beta)),
    ...
    def _log_loss_and_acc(self, epoch, loss, acc, suffix):
        summary = tf.compat.v1.Summary(value=[
            tf.compat.v1.Summary.Value(tag="loss_{}".format(suffix), simple_value=float(loss)),
            tf.compat.v1.Summary.Value(tag="acc_{}".format(suffix), simple_value=float(acc))
        ])
    ...
    def fit(self, X_train, y_train, X_valid, y_valid):
        ...
        with graph.as_default():
            ...
            loss_ops = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(
                labels=labels, logits=logits), name="loss")

            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate)
            train_ops = optimizer.minimize(loss_ops)

            correct = tf.equal(tf.argmax(input=prediction, axis=1), tf.argmax(input=labels, axis=1), name="correct")
            accuracy_ops = tf.reduce_mean(input_tensor=tf.cast(correct, tf.float32), name="accuracy")
            ...
            self.summary_writer = tf.compat.v1.summary.FileWriter(self.logs_path, graph=graph)
